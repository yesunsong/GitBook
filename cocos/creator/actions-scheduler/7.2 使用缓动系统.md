...
Cocos Creator 在 v2.0.9 提供了一套新的 API —— `cc.tween`。`cc.tween` 能够对对象的任意属性进行缓动，功能类似于 [cc.Action（动作系统）](https://docs.cocos.com/creator/manual/zh/scripting/actions.html)。但是 `cc.tween` 会比 `cc.Action` 更加简洁易用，因为 `cc.tween` 提供了链式创建的方法，可以对任何对象进行操作，并且可以对对象的任意属性进行缓动。

[动作系统](https://docs.cocos.com/creator/manual/zh/scripting/actions.html) 是从 Cocos2d-x 迁移到 Cocos Creator 的，提供的 API 比较繁琐，只支持在节点属性上使用，并且如果要支持新的属性就需要再添加一个新的动作。为了提供更好的 API，`cc.tween` 在 [动作系统](https://docs.cocos.com/creator/manual/zh/scripting/actions.html) 的基础上做了一层 API 封装。下面是 `cc.Action` 与 `cc.tween` 在使用上的对比：
