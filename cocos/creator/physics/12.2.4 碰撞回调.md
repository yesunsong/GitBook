...
1. <u>需要先在 [刚体组件](https://docs.cocos.com/creator/manual/zh/physics/physics/rigid-body.html) 中 **开启碰撞监听**，才会有相应的回调产生。</u>
...
![anatomy-aabbs](https://gitee.com/nlpleaf/PicGo/raw/master/20200622175040.png)

碰撞的过程

|                                                              |                                                              |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| 碰撞 1 ![img](https://gitee.com/nlpleaf/PicGo/raw/master/20200622175041.png)碰撞 2 ![img](https://gitee.com/nlpleaf/PicGo/raw/master/20200622175042.png)碰撞 3 ![img](https://gitee.com/nlpleaf/PicGo/raw/master/20200622175043.png) | 当两个碰撞体相互覆盖时，box2d 默认的行为是给每个碰撞体一个冲量去把它们分开，但是这个行为不一定能在一个时间步内完成。 像这里显示的一样，示例中的碰撞体会在三个时间步内相互覆盖直到“反弹”完成并且它们相互分离。  在这个时间里我们可以定制我们想要的行为，**onPreSolve** 会在每次物理引擎处理碰撞前回调，我们 可以在这个回调里修改碰撞信息，而 **onPostSolve** 会在处理完成这次碰撞后回调，我们可以在这个回调中获取到物理引擎计算出的碰撞的冲量信息。 下面给出的输出信息能使我们更清楚回调的顺序。`        ...    Step    Step    BeginContact    PreSolve    PostSolve    Step    PreSolve    PostSolve    Step    PreSolve    PostSolve    Step    EndContact    Step    Step    ... ` |
|                                                              |                                                              |
...
最主要的信息都包含在 **contact** 中，这是一个 `cc.PhysicsContact` 类型的实例，可以在 api 文档中找到相关的 API。contact 中比较常用的信息就是碰撞的位置和法向量，contact 内部是按照刚体的本地坐标来存储信息的，而我们一般需要的是世界坐标系下的信息，我们可以通过 `contact.getWorldManifold` 来获取这些信息。需要注意的是回调中的信息在物理引擎都是以缓存的形式存在的，所以信息只有在这个回调中才是有用的，不要在脚本里缓存这些信息，但可以复制这些信息到缓存中来使用。获得碰撞信息的方法见代码清单8-14所示。
...
- points
  ...
  ![world-manifold-points](https://gitee.com/nlpleaf/PicGo/raw/master/20200622175044.png)

  **注意**：不是每一个碰撞都会有两个碰撞点，在模拟的更多的情况下只会产生一个碰撞点，下面列举一些其他的碰撞示例。

  ![collision-points-1](https://gitee.com/nlpleaf/PicGo/raw/master/20200622175045.png)

  ![collision-points-2](https://gitee.com/nlpleaf/PicGo/raw/master/20200622175046.png)

  ![collision-points-3](https://gitee.com/nlpleaf/PicGo/raw/master/20200622175047.png)

- normal

  碰撞点上的法向量，由自身碰撞体指向对方碰撞体，指明解决碰撞最快的方向。

  ![world-manifold-normal](https://gitee.com/nlpleaf/PicGo/raw/master/20200622175048.png)
  ...
  ![world-manifold-normal-2](https://gitee.com/nlpleaf/PicGo/raw/master/20200622175049.png)
