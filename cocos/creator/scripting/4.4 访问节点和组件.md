...
由千 Cocos Creator是将组件脚本添加到节点，再由节点为索引调度生效的，凡 以，通常情况下所有组件脚本都有对应节点，获取 “node" 成员是一定有合理值的，因此这里并没有对取到的成员作合法性判断。
...
​	你也可以为 `getComponent` 传入一个类名。**<u>*对用户定义的组件而言，类名就是脚本的文件名，并且区分大小写。*</u>**例如 `SinRotate.js` 里声明的组件，类名就是 `SinRotate`。
...
![player-in-inspector-null](https://gitee.com/nlpleaf/PicGo/raw/master/3614d25a034016cd99122b21a85a551e)

​	接着你就可以将层级管理器上的任意一个节点拖到这个 Player 控件：

![player-in-inspector](https://gitee.com/nlpleaf/PicGo/raw/master/2f95e2fe30df315012c8d2f12999c436)
...
更详细内容，请参考 [模块化](https://docs.cocos.com/creator/manual/zh/scripting/modular-script.html)。